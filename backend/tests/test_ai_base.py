...
def make_view(*, street_index: int, call_amount: int, own_bet: int, big_blind: int = 100) -> dict:
    return {
        "your_player_id": "hero",
        "street_index": street_index,
        "big_blind": big_blind,
        "seats": [{"player_id": "hero", "bet": own_bet}],
        "legal_actions": {"call_amount": call_amount},
    }


def test_fold_is_always_talk_eligible() -> None:
    view = make_view(street_index=0, call_amount=100, own_bet=0)
